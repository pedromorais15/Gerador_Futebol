...
def generate_player_scout(jogador, esquema_proprio, estilo_time, caracteristicas):
    ...
    conteudo_prompt = (
        f"Analise o jogador ou grupo '{jogador}' levando em consideração as seguintes características individuais informadas: {lista_caracteristicas}. "
        ...
        f"Use exatamente um destes formatos de formação no campo 'formacao_recomendada': 4-3-3, 3-5-2, 4-4-2, 4-2-3-1 ou 5-3-2.\n\n"
        f"Para o campo 'escalacao_sugerida', monte obrigatoriamente uma escalação completa de 11 jogadores com posições genéricas "
        f"(ex: Goleiro, Lateral Esquerdo, Zagueiro, etc.) para CADA uma das 5 formações: 4-3-3, 3-5-2, 4-4-2, 4-2-3-1 e 5-3-2. "
        f"Justifique brevemente por que cada formação é ou não a ideal para este confronto. "
        f"As instruções táticas devem ser específicas para enfrentar o modelo de jogo informado."
    )
